...
import { inviteAdmin, validateAdminInvite } from "@/lib/admin/admins";
...
export async function POST(request: Request) {
  ...
  const validation = validateAdminInvite((body ?? {}) as Record<string, unknown>);
  if (!validation.ok) {
  ...
  try {
    ...
    const result = await inviteAdmin(supabase, validation.data, {
    ...
    if (status === 500) console.error("admin/admins POST failed", err);
