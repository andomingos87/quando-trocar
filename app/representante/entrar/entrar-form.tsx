...
export function RepresentanteEntrarForm() {
  ...
  const requestOtp = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch("/api/representante/auth/request-otp", {
        method: "POST",
  ...
  const verifyOtp = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setState({ status: "submitting" });
    try {
      const res = await fetch("/api/representante/auth/verify-otp", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
