...
- **Template obrigatório** (`WHATSAPP_TEMPLATE_CONVITE_REP_NAME`, categoria Marketing — boas-vindas/convite, `pt_BR`): o representante quase nunca está na janela de 24h, então texto livre seria rejeitado pela Meta. O template atualmente aprovado é `convite_representates`, com variáveis **posicionais**: `{{1}}` = primeiro nome e `{{2}}` = link do portal. Copy no runbook `docs/runbooks/meta-whatsapp-setup.md §12`. Enquadrar como aviso/boas-vindas: evitar termos de acesso/login no corpo ("código", "acesse", "entrar", "seu número de WhatsApp", "portal"), que a Meta lê como template de OTP. Categoria não afeta o código de envio.
