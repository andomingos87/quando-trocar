...
- **Dois modos de geração** ([ADR-0022](./adr/0022-modo-respond-grounded.md)): **rewrite** (CV1) reescreve o tom da enlatada, proibido de adicionar conteúdo; **respond** (CV2) responde a pergunta do usuário grounded **apenas** no bloco de conhecimento fechado (`lib/whatsapp/product-knowledge.ts` — fatos do produto sem preço — + FAQ do banco **filtrada por regex de preço** + nome da oficina + link de handoff). Respond exige `userMessage`, não se aplica a `vendas` (degrada para rewrite) e hoje só é acionado pela categoria `pergunta` da operação (§3.3). Protocolo "não sei": fora do conhecimento → `dontKnow=true` → enlatada (que na `pergunta` é handoff para humano).
...
- **Auditoria**: cada geração registra versão do prompt (`cv2-1`), **`generationMode` (`rewrite|respond`)**, intenção, `userMessage` truncado (~300 chars), aprovação/reprovação e uso de fallback em `agent_tool_calls`.
- **Protocolo "não sei"**: fora do conhecimento fornecido, admite e encaminha (no respond, a enlatada de fallback já é um handoff); a pergunta vira registro em `perguntas_sem_resposta` (tabela — fase futura).
