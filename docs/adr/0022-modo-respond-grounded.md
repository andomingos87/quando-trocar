...
6. **Vendas fora de escopo**: o agente de vendas segue 100% rewrite; `respond` com `agentMode="vendas"` degrada defensivamente para rewrite. *(Invariante emendado pela [ADR-0024](./0024-respond-em-vendas.md): o caso geral do `fora_escopo` de vendas passou a rodar em respond, grounded no conhecimento de vendas.)*
