...
- `.cursor/prompts/whatsapp-*.md`, `.codex/prompts/whatsapp-*.md`, `.cursor/prompts/openai-structured-output-review.md`
...
6. **Structured Outputs com enums fechados.** Toda chamada OpenAI usa Responses API com schema estrito. Enums têm que casar com as unions em `lib/whatsapp/types.ts`. Falha de JSON ou enum inesperado → fallback determinístico, nunca crash. Fonte: [`.cursor/prompts/openai-structured-output-review.md`](../../../.cursor/prompts/openai-structured-output-review.md).
...
2. **Prompt do agente** que você vai alterar:
   - `vendas` → [`.cursor/prompts/whatsapp-sales-agent.md`](../../../.cursor/prompts/whatsapp-sales-agent.md)
   - `onboarding` / `operacao` → [`.cursor/prompts/whatsapp-onboarding-agent.md`](../../../.cursor/prompts/whatsapp-onboarding-agent.md)
   - `cliente_final_lembrete` → [`.cursor/prompts/whatsapp-reminder-agent.md`](../../../.cursor/prompts/whatsapp-reminder-agent.md)
...
2. Edite o prompt em `.cursor/prompts/whatsapp-*.md` (espelhado em `.codex/prompts/`) E o reflexo dele em `lib/whatsapp/*-agent.ts` (prompt do código se houver, ou regras determinísticas).
3. Se o eval set existir (`tests/whatsapp-agent-evals/`), adicione o caso lá também.
4. Rode `npm test -- whatsapp` e confira regressão.
5. Atualize o prompt note (`.cursor/prompts/` / `.codex/prompts/`) com o que mudou — esse arquivo é o source-of-truth do comportamento.
...
- **Novo intent** num agente existente → adicionar na union em `types.ts`, no schema do Structured Output, no prompt em `.cursor/prompts/`, na regra determinística que valida a transição, e no eval set. Testes de classificação + testes de transição de status.
...
- [ ] Prompt em `.cursor/prompts/` / `.codex/prompts/` atualizado e coerente com o código.
